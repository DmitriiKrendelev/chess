...
public class EvaluationFunctionMoves implements EvaluationFunction<BoardEngine> {
	...
	public int value(BoardEngine board) {
		...
		final int[] movesOponent = board.calculateAllowedMovesOponent(MOVES_SELECTOR);
		
		final int value = value(moves, board);
		board.invert();
		final int valueOponent = value(movesOponent, board);
		board.invert();
		
		return value - valueOponent;
	}
