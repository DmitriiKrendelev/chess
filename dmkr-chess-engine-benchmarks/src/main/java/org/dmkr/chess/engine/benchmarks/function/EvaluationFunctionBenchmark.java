...
public class EvaluationFunctionBenchmark {
	...
	@Benchmark
	public void testEvaluationFunctionItemValues(Blackhole blackHole) {
		resetCache(BOARD);
		blackHole.consume(ITEM_VALUES_FUNCTION.value(BOARD));
	}
	
	@Benchmark
	public void testEvaluationFunctionItemPositions(Blackhole blackHole) {
		resetCache(BOARD);
		blackHole.consume(ITEM_POSITIONS_FUNCTION.value(BOARD));
	}
	
	@Benchmark
	public void testEvaluationFunctionItemMoves(Blackhole blackHole) {
		resetCache(BOARD);
		blackHole.consume(ITEM_MOVES_FUNCTION.value(BOARD));
	}
	
	@Benchmark
	public void testEvaluationFunctionQueenInTheCenter(Blackhole blackHole) {
		resetCache(BOARD);
		blackHole.consume(QUEEN_IN_THE_CENTER_FUNCTION.value(BOARD));
	}
	
	@Benchmark
	public void testEvaluationFunctionComposite(Blackhole blackHole) {
		resetCache(BOARD);
