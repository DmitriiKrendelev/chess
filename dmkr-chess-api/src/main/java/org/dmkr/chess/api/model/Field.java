package org.dmkr.chess.api.model;


import static com.google.common.base.Preconditions.checkArgument;
import static org.dmkr.chess.api.utils.BoardUtils.isOnBoard;
import static org.dmkr.chess.api.utils.BoardUtils.invertIndex;

import java.util.stream.Stream;

import lombok.Getter;

public enum Field {

	H1, H2, H3, H4, H5, H6, H7, H8,
	G1, G2, G3, G4, G5, G6, G7, G8,
	F1, F2, F3, F4, F5, F6, F7, F8,
	E1, E2,	E3, E4, E5, E6, E7, E8,
	D1, D2, D3, D4, D5,	D6, D7, D8,
	C1, C2, C3, C4,	C5, C6, C7, C8,
	B1, B2, B3, B4, B5, B6, B7, B8,
	A1, A2, A3, A4, A5, A6, A7, A8;
	
	private static final String XNAMES = "ABCDEFGH";
	private static final String YNAMES = "12345678";
	
	@Getter
	private final int x;
	@Getter
	private final int y;
	@Getter
	private final int index;

	private static final Field[] indexedFields;

	static {
        indexedFields = new Field[Field.values().length];
        for (Field field : values()) {
            indexedFields[field.index] = field;
        }
    }
	
	private Field() {
		final String name = name();

		this.x = XNAMES.indexOf(name.charAt(0));
		this.y = YNAMES.indexOf(name.charAt(1));
		this.index = x + (y << 3);
		
		checkArgument(isOnBoard(x, y));
	}

	public static char xName(int y) {
		return YNAMES.charAt(y);
	}
	
	public static char yName(int x) {
		return XNAMES.charAt(x);
	}
	
	public static Field resolve(int x, int y) {
		return resolve(x + (y << 3));
	}

	public static Field resolve(int index, boolean isInverted) {
		return resolve(invertIndex(index, isInverted));
	}
	
	public static Field resolve(int index) {
		return indexedFields[index];
	}
	
	public static Stream<Field> stream() {
		return Stream.of(values());
	}
}
